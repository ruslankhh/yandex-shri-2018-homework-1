...
data.forEach(data => {
	const { title, titleColor, image, description, size, channelName } = data;
	const type = `t${image ? 'i' : ''}${description ? 'd' : ''}`;
	const className = 'FeedItem';
	const feedItem = FeedItem.content.cloneNode(true)
		.querySelector(`.${className}`);
	const feedItemTitle = feedItem.querySelector(`.${className}__title`);
	const feedItemImage = feedItem.querySelector(`.${className}__image`);
	const feedItemDescription = feedItem.querySelector(`.${className}__description`);
	const feedItemChannelName = feedItem.querySelector(`.${className}__channelName`);
	...
	if (image) {
		const image2 = image.replace(/(\*.?)\.(\*.?)/g, '$1@2x.$2');
		const image3 = image.replace(/(\*.?)\.(\*.?)/g, '$1@3x.$2');
